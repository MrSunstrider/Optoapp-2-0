...
BEGIN
    ...
    -- proyeccion_caja: uses deduped pagos matching (venta_id, dispensacion_id, servicio_extra_id)
    WITH pagos_dedup AS (
    ...
    SELECT jsonb_build_object(
        'ingresos_esperados', COALESCE(SUM(v.monto_total - COALESCE(pd_total.total_pagado, 0)), 0),
    ...
    ) pd_total ON pd_total.venta_id_match = v.id
    WHERE v.optica_id = p_optica_id
      AND v.monto_total - COALESCE(pd_total.total_pagado, 0) > 0.005;
