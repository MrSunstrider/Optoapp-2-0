-- ============================================================================
-- Consolidated migration for Fixes 2.13, 2.20, 2.29
--
-- All three fixes touch the same RPCs, so we apply them in ONE migration
-- instead of three separate CREATE OR REPLACE that would overwrite each other.
--
-- Fix 2.13 — auth.uid() membership check
--   IF NOT app_private.is_optica_member(auth.uid(), p_optica_id) THEN
--       RAISE EXCEPTION 'Access denied';
--   END IF;
--
-- Fix 2.20 — COALESCE fallback for venta_id in proyeccion_caja
--   Already present in rpc_analisis_mensual (latest version).
--   Preserved as-is.
--
-- Fix 2.29 — BI role gate (admin/gerente only)
...
--   END IF;
--
-- Order of checks: membership first, role second.
-- ============================================================================

-- ============================================================================
-- 1. rpc_deudores — add security checks, preserve full body
-- ============================================================================
...
-- ============================================================================
-- 2. rpc_analisis_mensual — add security checks, preserve full 16-field body
--    (Fix 2.20 COALESCE already present in proyeccion_caja CTE)
-- ============================================================================
...
BEGIN
    ...
    -- Cash flow projection (Fix 2.20: COALESCE venta_id fallback already present)
    WITH pagos_dedup AS (
...
-- ============================================================================
-- 3. rpc_cierre_caja_resumen — add security checks, preserve full body
-- ============================================================================
...
-- ============================================================================
-- Re-grant permissions (all three RPCs: authenticated + service_role only)
-- ============================================================================
