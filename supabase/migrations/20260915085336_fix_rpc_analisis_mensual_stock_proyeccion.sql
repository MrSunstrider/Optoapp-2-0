-- JD reportes-cluster S5+I4:
-- Restore R23 stock_estancado (real ultima_venta / dias_sin_venta, no low-stock filter)
-- and NULL-safe proyeccion_caja so egresos_programados survive when unpaid ventas are empty.
-- Base body: 20260815010805; stock CTE from 20260709000003.
...
BEGIN
    ...
    SELECT COALESCE(SUM(monto), 0) INTO v_egresos_programados
    FROM public.gastos_operativos
    WHERE optica_id = p_optica_id
    ...
    SELECT jsonb_build_object(
        ...
        'egresos_programados', v_egresos_programados,
        'saldo_neto', 0
    ) INTO v_proyeccion
    FROM all_ventas v
    LEFT JOIN (
    ...
    -- Zero unpaid rows leaves SELECT INTO NULL; keep scheduled egresos.
    IF v_proyeccion IS NULL THEN
...
COMMENT ON FUNCTION public.rpc_analisis_mensual(TEXT, DATE) IS
  'Monthly BI JSON. stock_estancado uses real sales dates (R23); proyeccion_caja keeps egresos_programados when unpaid ventas are empty.';
