-- ============================================================================
-- Migration: Re-create rpc_adjust_montura_stock
--
-- This RPC was dropped in 20260716050000_cleanup_dead_rpcs.sql because the
-- Android app had not yet integrated it into the upload flow. Now that
-- UploadSyncCoordinator.uploadDispensaciones() will call it, the function
-- must be re-created exactly as defined in remote_schema_dump.sql:705.
--
-- SECURITY DEFINER ensures the RPC runs with owner privileges, bypassing RLS
-- so it can UPDATE monturas across any optica_id — the caller's optica_id is
-- validated by the WHERE clause and the JWT/RLS already restricts which optica
-- the authenticated user can operate on.
-- ============================================================================
