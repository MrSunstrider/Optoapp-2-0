-- ============================================================================
-- Phase 2 — Drop ventas table and rewrite RPCs to query source tables directly
-- ============================================================================
-- This migration completes the ventas→source refactor by:
--   1. Dropping triggers that synced disp/serv_extra → ventas
--   2. Dropping deprecated rpc_resumen_financiero
--   3. Rewriting recalcular_resumen_diario to UNION dispensaciones + servicios_extra
--   4. Rewriting rpc_deudores to UNION source tables
--   5. Rewriting rpc_count_pendientes to query source tables directly
--   6. Rewriting rpc_analisis_mensual proyeccion_caja section
--   7. Dropping public.ventas CASCADE
-- ============================================================================

-- ============================================================================
-- 2.1: DROP triggers and trigger functions that maintain ventas
-- ============================================================================
...
-- trg_pagos_before_insert_venta_id is kept (orphaned column, backward compat)

-- ============================================================================
-- 2.6: DROP deprecated rpc_resumen_financiero
-- ============================================================================
...
--      Replaces 3 ventas queries with UNION of dispensaciones + servicios_extra
-- ============================================================================
...
BEGIN
    -- Daily sales: UNION of dispensaciones + servicios_extra for the given date
    WITH daily_ventas AS (
        SELECT monto_total, 0::numeric AS costo_unitario_snapshot
    ...
    -- Daily payments (unchanged — queries pagos directly)
    ...
    -- Outstanding balance: UNION source tables instead of ventas
    WITH pagos_dedup AS (
        SELECT
    ...
    ON pd.venta_id_match=v.venta_id
    ...
    -- Upsert resumen_diario (unchanged)
...
-- ============================================================================
-- 2.3: REWRITE rpc_deudores — UNION source tables instead of ventas
--      DROP first because RETURNS TABLE changed (paciente_id was added)
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        v.venta_id, v.fecha, v.monto_total,
...
-- ============================================================================
-- 2.4: REWRITE rpc_count_pendientes — query source tables directly
-- ============================================================================
...
DECLARE
    ...
    v_servicios integer;
BEGIN
    -- Overdue deliveries: UNION of disp + serv_extra with 'Pendiente' estado
    WITH pendientes AS (
        SELECT 1 FROM public.dispensaciones
        WHERE optica_id = p_optica_id AND estado_entrega = 'Pendiente' AND fecha < CURRENT_DATE
        UNION ALL
        SELECT 1 FROM public.servicios_extra
        WHERE optica_id = p_optica_id AND estado = 'Pendiente' AND fecha < CURRENT_DATE
    )
    SELECT COUNT(*) INTO v_entregas FROM pendientes;

    -- Unpaid balance: UNION of all disp + serv_extra with insufficient payments
    WITH all_ventas AS (
        SELECT 'v_disp_' || id AS venta_id, monto_total
        FROM public.dispensaciones
        WHERE optica_id = p_optica_id
    ...
    pagos_agrupados AS (
        SELECT
            COALESCE(venta_id, 'v_disp_'||dispensacion_id, 'v_serv_'||servicio_extra_id) AS venta_id_match,
    ...
    INTO v_servicios
    FROM all_ventas v
    LEFT JOIN pagos_agrupados pg ON pg.venta_id_match = v.venta_id
    WHERE v.monto_total - COALESCE(pg.total_pagado, 0) > 0.005;
...
-- ============================================================================
-- 2.5: REWRITE rpc_analisis_mensual — proyeccion_caja section uses UNION
--      NOTE: rpc_deudores is called by this function and must exist first
-- ============================================================================
...
-- ============================================================================
-- 2.7: DROP ventas table (CASCADE drops RLS policies, indexes, FK)
-- ============================================================================

DROP TABLE IF EXISTS public.ventas CASCADE;

-- ============================================================================
-- Re-grant permissions for all rewritten functions
-- ============================================================================
