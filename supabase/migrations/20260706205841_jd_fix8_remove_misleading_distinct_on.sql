-- Fix #8: Remove misleading DISTINCT ON (pg.id) — pg.id is PK, no dedup needed.
-- The CTE selects directly from pagos (no ventas join), so each row is already unique.
...
-- 8b: rpc_analisis_mensual (only the proyeccion_caja CTE changes)
