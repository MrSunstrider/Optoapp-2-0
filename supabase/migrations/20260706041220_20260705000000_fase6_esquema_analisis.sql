...
ALTER TABLE public.ventas ADD COLUMN IF NOT EXISTS categoria_producto_id TEXT REFERENCES public.categorias_producto(id);
CREATE INDEX IF NOT EXISTS idx_ventas_categoria ON public.ventas (categoria_producto_id);
...
CREATE INDEX IF NOT EXISTS idx_costos_vigentes ON public.costos_productos (optica_id, categoria_producto_id) WHERE vigente_hasta IS NULL;
...
CREATE POLICY costos_productos_insert ON public.costos_productos FOR INSERT WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS costos_productos_update ON public.costos_productos;
CREATE POLICY costos_productos_update ON public.costos_productos FOR UPDATE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente'])) WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS costos_productos_delete ON public.costos_productos;
CREATE POLICY costos_productos_delete ON public.costos_productos FOR DELETE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin']));
...
CREATE POLICY configuracion_financiera_insert ON public.configuracion_financiera FOR INSERT WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS configuracion_financiera_update ON public.configuracion_financiera;
CREATE POLICY configuracion_financiera_update ON public.configuracion_financiera FOR UPDATE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente'])) WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS configuracion_financiera_delete ON public.configuracion_financiera;
CREATE POLICY configuracion_financiera_delete ON public.configuracion_financiera FOR DELETE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin']));
...
CREATE INDEX IF NOT EXISTS idx_gastos_opt_fecha ON public.gastos_operativos (optica_id, fecha);
...
CREATE POLICY gastos_operativos_insert ON public.gastos_operativos FOR INSERT WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS gastos_operativos_update ON public.gastos_operativos;
CREATE POLICY gastos_operativos_update ON public.gastos_operativos FOR UPDATE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente'])) WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));

DROP POLICY IF EXISTS gastos_operativos_delete ON public.gastos_operativos;
CREATE POLICY gastos_operativos_delete ON public.gastos_operativos FOR DELETE USING (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente']));
...
CREATE INDEX IF NOT EXISTS idx_margen_cat_opt_per ON public.margen_por_categoria (optica_id, periodo);
...
CREATE INDEX IF NOT EXISTS idx_resumen_diario_opt_fecha ON public.resumen_diario (optica_id, fecha);
...
CREATE POLICY feedback_recomendaciones_insert ON public.feedback_recomendaciones FOR INSERT WITH CHECK (app_private.has_optica_role(auth.uid(), optica_id, ARRAY['admin', 'gerente', 'especialista', 'asesor', 'asesora', 'ventas']));
...
BEGIN
    ...
    SELECT COALESCE(COUNT(*), 0), COALESCE(SUM(v.monto_total - COALESCE(pg.total_pagado, 0)), 0)
    INTO v_saldo_cantidad, v_saldo_total
    FROM public.ventas v
    LEFT JOIN (SELECT venta_id, SUM(monto) AS total_pagado FROM public.pagos WHERE optica_id = p_optica_id GROUP BY venta_id) pg ON pg.venta_id = v.id
    WHERE v.optica_id = p_optica_id AND v.monto_total - COALESCE(pg.total_pagado, 0) > 0.005;
