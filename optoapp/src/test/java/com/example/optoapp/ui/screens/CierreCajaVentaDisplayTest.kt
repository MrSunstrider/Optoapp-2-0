...
class CierreCajaVentaDisplayTest {
    ...
    @Test
    fun `servicioVentaTitle includes OT when present`() {
    ...
    @Test
    fun `servicioVentaTitle returns null OT line when OT blank`() {
        val serv = ServicioExtra(
            id = "s1",
    ...
    fun `filterPagoDisplayItems matches OT label`() {
        val item = com.example.optoapp.viewmodel.PagoDisplayItem(
            pago = com.example.optoapp.data.Pago(
                id = "p1",
                fecha = today,
                tipo = "Efectivo",
                monto = 100.0,
