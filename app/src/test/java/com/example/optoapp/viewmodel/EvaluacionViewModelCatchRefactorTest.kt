...
/**
 * Approval tests for [EvaluacionViewModel.parseSnellenToLogMar] behavior
 * BEFORE catch refactoring. These must pass before AND after changing
 * `catch (e: Exception)` to `catch (e: NumberFormatException)`.
 *
 * The function is a pure string→Double? parser extracted as a companion
 * object function specifically to enable direct unit testing.
 */
class EvaluacionViewModelCatchRefactorTest {
    ...
    fun `parseSnellenToLogMar valid input returns logMAR`() {
        ...
        val result = EvaluacionViewModel.parseSnellenToLogMar("20/20")
    ...
    fun `parseSnellenToLogMar 20 divided by 40 returns correct logMAR`() {
        ...
        val result = EvaluacionViewModel.parseSnellenToLogMar("20/40")
        assertNotNull(result)
    ...
    fun `parseSnellenToLogMar 20 divided by 200 returns logMAR 1`() {
        ...
        val result = EvaluacionViewModel.parseSnellenToLogMar("20/200")
        assertNotNull(result)
    ...
    fun `parseSnellenToLogMar empty string returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar(""))
    }
    ...
    fun `parseSnellenToLogMar no slash returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar("2020"))
    }

    @Test
    fun `parseSnellenToLogMar non numeric returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar("20/abc"))
    }

    @Test
    fun `parseSnellenToLogMar zero denominator returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar("20/0"))
    }

    @Test
    fun `parseSnellenToLogMar negative denominator returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar("20/-5"))
    }

    @Test
    fun `parseSnellenToLogMar three parts returns null`() {
        assertNull(EvaluacionViewModel.parseSnellenToLogMar("20/20/20"))
    }

    @Test
    fun `parseSnellenToLogMar 20 divided by 63 returns approx logMAR 0_5`() {
        // 20/63 ≈ 0.317 → -log10(0.317) ≈ 0.498
        val result = EvaluacionViewModel.parseSnellenToLogMar("20/63")
        assertNotNull(result)
    ...
    fun `parseSnellenToLogMar 20 divided by 400 returns logMAR 1_3`() {
        ...
        val result = EvaluacionViewModel.parseSnellenToLogMar("20/400")
        assertNotNull(result)
