...
export function normalizeSphere(value: string): string {
  const v = value.trim().toLowerCase();
  return (v === "plano" || v === "neutro" || v === "pl") ? "0.00" : value;
}

export function calculateEE(esf: string, cil: string): number {
  const e = parseFloat(esf) || 0;
  const c = parseFloat(cil) || 0;
  return e + (c / 2.0);
}

export function transpose(esf: string, cil: string, eje: string) {
  const c = parseFloat(cil) || 0;
...
export class EvaluacionBuilder {
  ...
  withRefraccionOd(esf: string, cil: string, eje: string) {
    const normEsf = normalizeSphere(esf);
    const { nEsf, nCil, nEje } = transpose(normEsf, cil, eje);
  ...
  private updateAutoDiagnosis() {
    const eeOd = calculateEE(this._evaluacion.receta_od_esf || "0", this._evaluacion.receta_od_cil || "0");
    const eeOi = calculateEE(this._evaluacion.receta_oi_esf || "0", this._evaluacion.receta_oi_cil || "0");
