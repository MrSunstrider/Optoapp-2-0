...
- **`stock_estancado`** (JSONB array): unsold products from `monturas` LEFT JOIN `montura_movimientos (tipo='SALIDA_VENTA')` and `dispensaciones.montura_id`. `dias_sin_venta` SHALL be `CURRENT_DATE - MAX(fecha)` for sold, 999 for never-sold. `ultima_venta` SHALL be the real date or null. The low-stock filter (`stock_actual <= stock_minimo`) SHALL be removed. A new migration MUST `CREATE OR REPLACE` the current function body (preserving pago_effect / converge paths) and only patch stock + proyeccion sections.
