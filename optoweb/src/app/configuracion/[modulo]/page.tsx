...
import { DataManagementSection } from "@/components/config/backup-card";
...
async function SubscriptionLimitsSection({ opticaId }: { opticaId: string }) {
  const supabase = await createClient();
  const [limits, userCount, branchCount, optica] = await Promise.all([
    ...
    supabase.from("opticas").select("plan_code").eq("id", opticaId).maybeSingle()
  ]);
  const patientState = stateByUsage(limits.pacientesActuales, limits.maxPacientes);
  const isPro = optica?.data?.plan_code === "pro_individual";
  return (
    <section className="rounded-3xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <LimitRow
          ...
          max={isPro ? null : limits.maxPacientes}
          state={isPro ? "normal" : patientState}
        />
        <LimitRow
      ...
      {isPro ? (
        <div className="mt-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-4 text-center">
          <p className="text-xs font-black uppercase tracking-widest text-emerald-500">PRO activo — pacientes ilimitados</p>
        </div>
      ) : (
        <form action={activarProAction} className="mt-6">
          <input type="hidden" name="opticaId" value={opticaId} />
          <button className="w-full rounded-2xl bg-primary py-4 text-xs font-black uppercase tracking-[0.2em] text-primary-foreground shadow-xl shadow-primary/20 hover:scale-[1.01] transition-all active:scale-95">
            Activar PRO (alpha)
          </button>
        </form>
      )}
      <p className="mt-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center">
        Modo alpha — el billing real se integrará tras las pruebas.
      </p>
    </section>
  );
}
...
async function activarProAction(formData: FormData) {
  "use server";
  const activeOptica = await getActiveOpticaContext();
  if (!activeOptica) redirect("/seleccion-optica");
  ...
  const { error } = await supabase
    .from("opticas")
    .update({ plan_code: "pro_individual", max_pacientes_por_optica: null })
    .eq("id", opticaId);
  if (error) redirect("/configuracion/suscripciones?error=No se pudo activar PRO");
  redirect("/configuracion/suscripciones?msg=PRO activado — pacientes ilimitados");
}
