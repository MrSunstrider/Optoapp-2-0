...
function ultimoApellido(nombreCompleto: string): string {
  const partes = nombreCompleto.split(/\s+/).filter(Boolean);
  return partes.length >= 2 ? partes[1] : partes[0] ?? "";
}

function tituloPorSexo(sexo: string | null): string {
  switch (sexo?.toLowerCase()) {
    case "masculino": case "m": return "Sr.";
    case "femenino": case "f": return "Sra.";
    default: return "Sr./Sra.";
  }
}
            ...
            <div className="absolute right-0 top-full z-50 mt-2 min-w-[280px] rounded-2xl border border-border bg-card py-2 text-sm text-foreground shadow-2xl animate-in fade-in slide-in-from-top-2">
              ...
              {proximaCitaIso && recordatorioCita && (
                <a
                  ...
                  onClick={() => setMenuOpen(false)}
                >
                  Recordar Proxima Cita ({fechaCitaLabel})
                </a>
              )}
              <hr className="border-border/50 my-1" />
              <a
                className="block px-4 py-2.5 font-medium hover:bg-primary/10 hover:text-primary transition-colors"
                href={waUrl(digits, `Hola ${firstTokenName(nombrePaciente)}, le recordamos que tiene pendiente el recojo de sus lentes. Lo esperamos en nuestra optica en nuestro horario de atencion.`)}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setMenuOpen(false)}
              >
              ...
              <a
                className="block px-4 py-2.5 font-medium hover:bg-primary/10 hover:text-primary transition-colors"
                href={waUrl(digits, `Buen dia. ${tituloPorSexo(sexo ?? null)} ${ultimoApellido(nombrePaciente)}, sus lentes ya estan listos, puede venir a recogerlos en este horario: Martes a Sabado de 10am a 6:30pm y Domingos de 10am a 2pm. Lo esperamos.`)}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setMenuOpen(false)}
              >
